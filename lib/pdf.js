...
class PDF {
    constructor(data, options) {
        this.config = deepCloneMerge(defaults, options.opts);
    ...
    cleanTagStyle(options) {
        ...
        if (options.font) {
            ...
            if (options.font.indexOf('/') >= 0) options.font = path.resolve(this.config.opts.basePath, options.font);
        }
    ...
    getDocHeight() {
        this.config.document.size = [148, 10000];
        this.config.document.margins = { // by default, all are 72
            top: 0,
            bottom: 0,
            left: 0,
            right: 0
    ...
    render() {
        ...
        if (this.config.size) {
           if (typeof this.config.size == 'object') {
             if (this.config.size[1] == 'auto') { // check if dynamic height
                this.config.document.size = [148, parseInt(this.getDocHeight()) ];
             }
             else {
                this.config.document.size = this.config.size;
             }
           }
        }
        console.log(this.config.document.size)
        this.config.document.margins = { // by default, all are 72
            top: 0,
            bottom: 0,
            left: 0,
